...
def Inceptionv3(img_input, ratio = 1, num_A = 3, num_B = 4, num_C = 2, num_class = 1000, dropout = 0.5):
    ...
    x = inceptionv4.moduleB(ratio, x, num = 128//ratio)    
    for i in range(num_B - 2):
        x = inceptionv4.moduleB(ratio, x, num = 160//ratio)    
    x = inceptionv4.moduleB(ratio, x, num = 192//ratio)
    
    x = inceptionv4.resizeB(ratio, x)
    
    for i in range(num_C):
        x = inceptionv4.moduleC(ratio, x)        
